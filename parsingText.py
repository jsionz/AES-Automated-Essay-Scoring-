...
def return_essay_object(file_name):
    ...
    with open(file_name, 'r', errors='ignore') as f:
        line = f.readline()
        i=0
        with open(r'./rdata/object_essay_allData_39421.pkl','wb') as f_pkl_object:
            while line:
                paragraphs = []
                list_essay_info = line.split('\t')
...
if  __name__ == "__main__":
    ...
    # score_list = []
    # model = joblib.load(r'./model/basic_liner_model.pkl')
    # essay_object = essay.Essay()  # 创建essay对象
    # fs = Features()  # 创建特征对象
    # tnt_tagger1, t_tagger_brown1 = token_pos_train()
    # essays_object_list = []
    # with open(r'./train_test_set/pigaiessay.test', 'r', errors='ignore') as f:
    #     line = f.readline()
    #     while line:
    #         paragraphs = []
    ...
    #         essay_object.sentences = nltk.sent_tokenize(essay_extracted)  # 文章的句子列表
    #         essay_object.tokens = nltk.word_tokenize(essay_extracted)  # 文章的token列表
    #         essay_object.tokens_pos_lemma = parse_token_pos_lemma(tnt_tagger1, t_tagger_brown1,
    #                                                               essay_object)  # 文章token,pos,lemma  注释：[0,1,2]-->[token,pos,lemma]
    #         essay_object.paragraphs_num = len(paragraphs)  # 文章的段落数
    #         essay_object.essay_words_num = len(nltk.word_tokenize(essay_extracted))  # 文章的长度
    #
